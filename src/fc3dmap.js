 ...
 * @param opt{Object}
 * @param opt.config {Object} 一些配置信息可参考源码config/config.js文件
 *
 * @constructor
 */
function Fc3dmap(elm, opt){
     ...
     * @type 配置信息
     */
    this.config=opt.config||cesiumConfig;
    /**
     *
...
/*        var bubble=document.createElement('div');
        ...
        bubble.className='cesium-viewer-selectionIndicatorContainer';
        bubble.innerHTML='<div id="popup" class="ol-popup">\n' +
            '        <a href="#" id="popup-closer" class="ol-popup-closer" onclick="closePopup()"></a>\n' +
            '        <div id="popup-content"><p>测试气泡</p></div>\n' +
            '    </div>';*/
        _self.viewer = new Cesium.Viewer(elm,{
            ...
            infoBox:false,
            imageryProvider:!opt.imageryProvider&&_self.config.mapUrl&&_self.config.mapUrl!==''&& new TDTMapImageryProvider({url: _self.config.mapUrl})
        });
       // _self.viewer._container.appendChild(bubble);
        _self.viewer._cesiumWidget._creditContainer.style.display = "none";
        //_self.viewer.selectionIndicator=new Cesium.SelectionIndicator(bubble,_self.viewer.scene);
        return _self.viewer;
    };
    /**
     * 飞入效果
